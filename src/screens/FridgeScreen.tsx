...
import { FoodTile, DdayBadge, StockTag } from '../components/ui';
import { STORAGE_LABEL, CATEGORY, STOCK, STOCK_ORDER, SORT_OPTIONS, StockLevel } from '../data/constants';
import { useApp, FridgeItem } from '../data/store';
import { useNav } from '../navigation/nav';

// 3개 보관 위치 탭. 실온 탭에는 양념/소스·기타 보관 재료도 함께 묶어 보여준다.
...
export function FridgeScreen() {
  ...
  const sortFn = (a: FridgeItem, b: FridgeItem) => {
    if (sort === '유통기한 임박순') return (a.dday ?? 9999) - (b.dday ?? 9999);
    if (sort === '이름순') return a.name.localeCompare(b.name, 'ko');
    if (sort === '남은 정도순') return STOCK_ORDER.indexOf(b.stock) - STOCK_ORDER.indexOf(a.stock);
    return 0;
  };
  const listFor = (i: number) =>
    fridge.filter((x) => TABS[i].match(x.storage) && x.name.includes(query.trim())).sort(sortFn);
  ...
  const renderCard = (it: FridgeItem) => (
    <Pressable key={it.id} style={s.itemCard} onPress={() => setSheet(it)}>
      <FoodTile name={it.name} category={it.category} size={46} />
      <View style={{ flex: 1 }}>
  ...
  return (
    <View style={s.root}>
      <View style={s.header}>
        ...
        <Pressable style={s.addBtn} onPress={() => nav.openIngredientForm()}>
          <Icon name="plus" size={16} color={colors.white} weight="bold" />
          <Text style={s.addBtnText}>식재료 추가</Text>
        </Pressable>
      </View>

      <View style={s.searchRow}>
        <Icon name="search" size={18} color={colors.inkAsst} />
        <TextInput value={query} onChangeText={setQuery} placeholder="식재료 검색" placeholderTextColor={colors.inkAsst} style={s.search} />
      </View>
      ...
      <View style={s.tabs}>
        {TABS.map((t, i) => {
          const on = i === tab;
          return (
            <Pressable key={t.label} style={s.tab} onPress={() => goTab(i)}>
              <View style={s.tabInner}>
                <Icon name={t.icon} size={16} color={on ? colors.primary : colors.inkAsst} weight={on ? 'fill' : 'regular'} />
                <Text style={[s.tabText, { color: on ? colors.ink : colors.inkAsst }]}>{t.label}</Text>
              </View>
              <View style={[s.underline, on && s.underlineOn]} />
            </Pressable>
          );
        })}
      </View>
      ...
      <View style={s.sortRow}>
        <Text style={s.countText}>{listFor(tab).length}개</Text>
        <Pressable style={s.sortBtn} onPress={() => setSortIdx((i) => (i + 1) % SORT_OPTIONS.length)}>
          <Icon name="funnel" size={14} color={colors.inkAlt} weight="bold" />
        ...
        {w > 0 && (
          ...
          >
            {TABS.map((t, i) => {
              ...
              return (
                <ScrollView key={t.label} style={{ width: w }} contentContainerStyle={{ paddingHorizontal: 20, paddingVertical: 6, paddingBottom: 24 }} showsVerticalScrollIndicator={false}>
                  {list.map(renderCard)}
...
const s = StyleSheet.create({
  ...
  tabs: { flexDirection: 'row', marginTop: 14, marginHorizontal: 20, borderBottomWidth: 1, borderBottomColor: colors.line },
  tab: { flex: 1, alignItems: 'center' },
  tabInner: { flexDirection: 'row', alignItems: 'center', gap: 6, paddingTop: 4, paddingBottom: 10 },
  tabText: { fontFamily: font.bold, fontSize: 15 },
  underline: { height: 2.5, width: '60%', borderRadius: 2, backgroundColor: 'transparent', marginBottom: -1.5 },
  underlineOn: { backgroundColor: colors.primary },

  sortRow: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', paddingHorizontal: 24, paddingTop: 10, paddingBottom: 4 },
  countText: { fontFamily: font.semibold, fontSize: 13, color: colors.inkAsst },
  sortBtn: { flexDirection: 'row', alignItems: 'center', gap: 5 },
  sortText: { fontFamily: font.semibold, fontSize: 13, color: colors.inkAlt },

  itemCard: { flexDirection: 'row', alignItems: 'center', gap: 13, backgroundColor: colors.surface, borderRadius: radius.lg, borderWidth: 1, borderColor: colors.line, padding: 12, marginBottom: 9 },
  itemTop: { flexDirection: 'row', alignItems: 'center', gap: 8 },
  itemName: { fontFamily: font.bold, fontSize: 16, color: colors.ink },
  itemMeta: { fontFamily: font.medium, fontSize: 12.5, color: colors.inkAlt, marginTop: 3 },
